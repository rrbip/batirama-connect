@props(['chunks', 'showActions' => false, 'showSelection' => false, 'selectedChunks' => [], 'editingChunkId' => null])
...
@else
    ...
    <div class="space-y-4">
        @foreach($chunks as $chunk)
            <div class="p-4 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 {{ $showSelection && in_array($chunk->id, $selectedChunks) ? 'ring-2 ring-primary-500' : '' }}">
                {{-- Chunk header --}}
                <div class="flex items-center justify-between mb-3 pb-3 border-b border-gray-100 dark:border-gray-700">
                    ...
                    {{-- Action buttons (only in ManageChunks page with Livewire context) --}}
                    @if($showActions && $editingChunkId !== $chunk->id)
