...
## 28. Optimisation des Ressources LLM

### 28.1 Problématique

Dans le mode Apprentissage Accéléré, si l'agent clique sur "Passer" après avoir vu le message client, la requête LLM est souvent **déjà en cours ou terminée**.

**Gaspillage potentiel :**
- Tokens consommés inutilement (coût API pour Gemini, OpenAI, Claude)
- Ressources GPU mobilisées (Ollama local)
- Temps de génération perdu

### 28.2 Solutions Proposées

#### Option A : Génération Différée ("Lazy Generation")

**Concept :** Ne pas générer automatiquement la réponse IA. L'agent choisit d'abord son mode d'action.

```
┌─────────────────────────────────────────────────────────────────┐
│ 💬 Nouveau message client :                                     │
│ "Comment configurer le module XYZ ?"                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Que souhaitez-vous faire ?                                     │
│                                                                 │
│  [🤖 Demander une suggestion IA]  [✍️ Répondre directement]    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**Workflow :**
1. Message client arrive → Pas de génération IA immédiate
2. Agent voit le message et choisit :
   - "Demander une suggestion IA" → Lance la génération
   - "Répondre directement" → Zone libre s'ouvre (pas de génération)
3. Si suggestion demandée → Flow normal (Valider/Corriger/Refuser)

**Avantages :**
- 0 gaspillage si l'agent répond directement
- L'agent garde le contrôle

**Inconvénients :**
- Ajoute une étape de décision
- Latence supplémentaire si l'agent veut la suggestion

**Configuration suggérée :**
```json
{
    "accelerated_learning_config": {
        "generation_mode": "lazy", // ou "immediate"
        ...
    }
}
```

#### Option B : Annulation de Job Avant Exécution

**Concept :** Permettre d'annuler un Job en attente dans la queue.

```php
// ViewAiSession.php
public function skipBeforeGeneration(int $messageId): void
{
    $message = AiMessage::findOrFail($messageId);

    // Marquer le message comme "skipped" AVANT que le Job ne s'exécute
    $message->update([
...
public function handle(): void
{
    ...
    // Vérifier si le message a été skippé entre-temps
    if ($message->processing_status === 'skipped') {
        Log::info('AI generation skipped by agent', ['message_id' => $this->messageId]);
...
- Efficace si queue longue ou agent rapide

**Inconvénients :**
- Race condition : si le Job démarre juste avant le skip
- Inefficace si queue rapide (Job déjà en cours)

#### Option C : Streaming avec Interruption (Recommandée)

**Concept :** Utiliser le streaming LLM et permettre d'interrompre en cours de génération.

**Compatibilité par provider :**

| Provider | Streaming | Interruption possible |
|----------|-----------|----------------------|
| **Ollama** | ✅ Oui | ✅ Oui (HTTP cancel) |
| **OpenAI** | ✅ Oui | ✅ Oui (SSE close) |
| **Gemini** | ✅ Oui | ✅ Oui (stream close) |
| **Claude** | ✅ Oui | ✅ Oui (stream close) |
| **Mistral** | ✅ Oui | ✅ Oui |
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. Message client arrive                                       │
│  2. Job dispatché avec stream_id unique                        │
│  3. Affichage "Génération en cours..." avec [⏹️ Annuler]        │
│                                                                 │
│  Si agent clique [Annuler] ou [Passer] :                       │
│  → Broadcast event "cancel_stream:{stream_id}"                  │
│  → Worker reçoit l'event et interrompt le stream               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```php
// StreamableAiGenerationJob.php
class StreamableAiGenerationJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Écouter les événements d'annulation via Redis
        Redis::subscribe(["cancel_stream:{$this->streamId}"], function () {
            $this->cancelled = true;
        });

        // Démarrer le streaming
        $stream = $this->llmService->streamGenerate($prompt);

        $fullResponse = '';
        foreach ($stream as $chunk) {
            // Vérifier si annulation demandée
            if ($this->cancelled) {
                Log::info('Stream cancelled by agent', ['stream_id' => $this->streamId]);
                $message->update(['processing_status' => 'cancelled']);
                return;
            }

            $fullResponse .= $chunk;

            // Optionnel : broadcast le chunk pour affichage temps réel
            broadcast(new StreamChunk($this->streamId, $chunk));
        }
...
// ViewAiSession.php
public function cancelGeneration(string $streamId): void
{
    // Publier l'événement d'annulation
    Redis::publish("cancel_stream:{$streamId}", json_encode([
    ...
    Notification::make()
        ...
        ->body('Vous pouvez maintenant répondre librement.')
        ->info()
        ->send();
}
```
...
**Avantages :**
- Économie immédiate dès l'annulation (pas de tokens supplémentaires)
- Fonctionne avec tous les providers qui supportent le streaming
- Expérience utilisateur fluide

**Inconvénients :**
- Plus complexe à implémenter
- Nécessite Redis ou système de pub/sub

#### Option D : Génération Hybride (Pragmatique)

**Concept :** Combiner les approches selon le contexte.

```
┌─────────────────────────────────────────────────────────────────┐
│ Si queue.size > 10 OU heure_creuse = false :                   │
│   → Mode LAZY (génération à la demande)                         │
│                                                                 │
│ Sinon :                                                         │
│   → Mode IMMEDIATE avec bouton [Annuler] pendant génération    │
└─────────────────────────────────────────────────────────────────┘
```

### 28.3 Recommandation

Pour le MVP, implémenter dans cet ordre :

1. **Phase 1 :** Option B (annulation de Job) - Simple, couvre les cas où la queue est lente
2. **Phase 2 :** Option A (Lazy) comme config optionnelle pour les agents à fort volume
3. **Phase 3 :** Option C (Streaming cancel) pour l'optimisation maximale

### 28.4 Interface Utilisateur avec Annulation
...
│ │  ████████░░░░░░░░░░░░ 40%                                  │  │
│ │                                                             │  │
│ │  [⏹️ Annuler et répondre moi-même]                         │  │
│ │                                                             │  │
│ └─────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│ ┌─ Assistant IA ──────────────────── [⏹️ Annulée] ───────────┐  │
│ │ Génération interrompue par l'agent                          │  │
│ └─────────────────────────────────────────────────────────────┘  │
│                                                                 │
│ ┌─ Votre réponse ─────────────────────────────────────────────┐  │
│ │ [Zone de texte libre]                                       │  │
...
## 29. Plan d'Implémentation (Additionnel)
...
### Phase 7 : Optimisation Ressources LLM (2-3 jours)
- [ ] Implémenter annulation de Job (Option B)
- [ ] Ajouter config `generation_mode: lazy|immediate`
- [ ] Interface avec bouton Annuler pendant génération
- [ ] (Optionnel) Streaming avec interruption pour Ollama

**Durée estimée totale révisée : 17-23 jours**
